...
const { nowUnix, cases } = fixtures as unknown as {
  nowUnix: number;
  cases: FixtureCase[];
};
...
function run(c: FixtureCase, store: TlogStateStore) {
  return verifyTlogProof(
    c.tlogProof,
    new TextEncoder().encode(c.statement),
    c.address,
    c.policy,
    {
  ...
  for (const c of cases) {
    it(c.name, async () => {
      ...
      if (c.expect === "ok") {
        ...
        expect(details.validWitnessCount).toBeGreaterThanOrEqual(
          c.policy.witnessThreshold,
        );
