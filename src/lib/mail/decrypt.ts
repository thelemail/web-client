...
export async function decryptPreview(
  ...
  try {
    ...
    throw new DecryptionError(
      "invalid_ciphertext",
      "preview is not valid JSON",
    );
  }
}

export function decryptPreviewRaw(
  accountId: string,
  encryptedPreviewB64: string,
): Promise<string> {
...
export async function unwrapPgpMime(
  ...
  for (let depth = 0; depth < 3; depth++) {
    ...
    if (!("plaintext" in res)) {
      throw new DecryptionError(
        "invalid_ciphertext",
        "expected text plaintext",
      );
    }
    current = res.plaintext;
    if (res.signature && res.signature.state !== "none")
...
export async function decryptBodyFromUrl(
  ...
  if (!("plaintext" in res)) {
    throw new DecryptionError(
      "invalid_ciphertext",
      "expected text plaintext for body",
    );
  }
