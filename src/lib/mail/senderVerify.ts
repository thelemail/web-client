...
const externalCache = new Map<
  string,
  { value: ExternalKeyState | null; at: number }
>();
...
export async function externalKeyState(
  senderAddress: string,
): Promise<ExternalKeyState | null> {
  const address = senderAddress.trim().toLowerCase();
  if (!address) return null;
  ...
  try {
    ...
    value = {
      ...
      firstSeenAtMillis: trust.firstSeenAt
        ? Date.parse(trust.firstSeenAt)
        : undefined,
    };
  } catch {
    value = null;
