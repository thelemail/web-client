...
async function hydrateEntry(
  ...
  try {
    ...
    const fromDisplay =
      preview.sender.display || preview.sender.address || "Unknown";
    ...
    const toAddresses = preview.recipients
      .filter((r) => r.kind === "to")
      .map((r) => r.address);
    return {
      id: item.id,
...
export async function hydrateThread(
  ...
  const seed =
    resp.items.find((it) => it.id === messageId) ??
    resp.items[resp.items.length - 1];
