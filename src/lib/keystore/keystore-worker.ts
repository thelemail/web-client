  ...
  cryptoReady = (async () => {
    ...
    try {
      ...
      if (
        ...
        !/already initialised/i.test(err.message)
      ) {
        throw err;
      }
    }
...
function handleVerifyLoginProof(
  args: VerifyLoginProofArgs,
): VerifyLoginProofResponse {
...
function handleVerifyRecoveryProof(
  args: VerifyRecoveryProofArgs,
): VerifyRecoveryProofResponse {
...
async function handleCommitPasswordChange(
  ...
  if (
    !pending ||
    pending.accountId !== args.accountId ||
    !pending.newCredentials
  ) {
    return { ok: false, code: "no_pending_change" };
  }
  ...
  try {
    ...
    console.warn(
      "keystore: commitPasswordChange persist failed, dropping stale record",
      err,
    );
    persisted = false;
...
async function handleInvalidatePersistedVault(
  args: InvalidatePersistedVaultArgs,
): Promise<void> {
  cachedRecords.delete(args.accountId);
  try {
    await deleteVault(args.accountId);
  ...
  return crypto.subtle.deriveKey(
    {
      name: "HKDF",
      hash: "SHA-256",
      salt: serverHalf as BufferSource,
...
async function handleEnrollPersistent(
  args: EnrollPersistentArgs,
): Promise<void> {
  const v = vaults.get(args.accountId);
  if (!v) {
  ...
  const wrapVersion: 1 | 2 =
    v.authScheme === "opaque_v1" && v.masterKey ? 2 : 1;
...
async function handleTryRestoreFromPersistent(
  ...
  await restorePromise;
  const rec =
    cachedRecords.get(args.accountId) ??
    (await getVault(args.accountId)) ??
    undefined;
...
async function handleGetPublicKey(
  ...
  const fingerprint =
    typeof fp === "string"
      ? hexToBytes(fp)
      : new Uint8Array(fp as ArrayLike<number>);
  return {
    ok: true,
...
async function handleCommitReformattedKey(
  ...
  try {
    ...
    if (prev) {
      ...
      if (
        v.wrapKey &&
        prev.wrappedPrivateKey &&
        prev.wrapPayloadVersion !== 2
      ) {
...
async function handleEncrypt(args: EncryptArgs): Promise<EncryptResponse> {
  ...
  let recipientKey;
  try {
    recipientKey = await openpgp.readKey({
      armoredKey: args.recipientPublicKeyArmored,
    });
  } catch (err) {
...
async function handleEncryptToKeys(
  args: EncryptToKeysArgs,
): Promise<EncryptToKeysResponse> {
  const v = vaults.get(args.accountId);
  if (!v) {
    return { ok: false, code: "locked" };
  }
  ...
  try {
    recipientKeys = await Promise.all(
      args.recipientPublicKeysArmored.map((armoredKey) =>
  ...
  if (args.verificationKeysArmored?.length) {
    try {
      verificationKeys = (await Promise.all(
        args.verificationKeysArmored.map((armoredKey) =>
          openpgp.readKey({ armoredKey }),
        ),
  ...
  try {
    const message = args.ciphertextArmored
      ...
      : await openpgp.readMessage({
          binaryMessage: args.ciphertextBinary as Uint8Array,
        });
    if (args.binary) {
      ...
      return {
        ...
        signature: wanted
          ? await verdictFor(signatures, verificationKeys)
          : undefined,
      };
    }
    const { data, signatures } = await openpgp.decrypt({
...
async function handleDisablePersistent(
  args: DisablePersistentArgs,
): Promise<void> {
...
async function handleOpaqueStartRegistration(
  ...
  return {
    operationId,
    registrationRequest: fromOpaqueWire(registrationRequest),
  };
}
...
async function handleOpaqueFinishRegistration(
  ...
  const { registrationRecord, exportKey } = opaqueClient.finishRegistration({
    ...
    identifiers: {
      client: clientIdentity(args.accountId, false),
  ...
  const encryptedPriv = await openpgp.encryptKey({
    privateKey: privObj,
...
async function handleOpaqueStartAuth(
  args: OpaqueStartAuthArgs,
): Promise<OpaqueStartAuthResponse> {
  await opaqueReady;
  reapOpaqueOps();
...
async function handleOpaqueFinishAuth(
  args: OpaqueFinishAuthArgs,
): Promise<OpaqueFinishAuthResponse> {
  const op = opaqueOps.get(args.operationId);
  ...
  const result = opaqueClient.finishLogin({
    ...
    password: op.password,
    identifiers: {
      client: clientIdentity(args.accountId, recovery),
      server: SERVER_IDENTITY,
    },
    keyStretching: KEY_STRETCHING,
  });
...
async function handleOpaqueCompleteLoginUnlock(
  ...
  try {
    ...
    const derivedMasterKeyId = opaqueBytesToBase64(
      await deriveMasterKeyId(amk),
    );
...
async function handleMigrationFinishStage(
  ...
  const masterKeyId = await deriveMasterKeyId(amk);
  const pgpPassphrase = await derivePgpPassphrase(amk);
  const stagedEncrypted = await openpgp.encryptKey({
    privateKey: v.privateKey,
    passphrase: pgpPassphrase,
  });
  amk.fill(0);
...
async function handleOpaqueRecoverySetupFinish(
  ...
  const { registrationRecord, exportKey } = opaqueClient.finishRegistration({
    ...
    clientRegistrationState: op.clientState,
    identifiers: {
      client: clientIdentity(args.accountId, true),
      server: SERVER_IDENTITY,
    },
    keyStretching: KEY_STRETCHING,
  });
  ...
  const exportKeyBytes = base64UrlToBytes(exportKey);
  const wrappedMasterKey = await wrapMasterKey(
    exportKeyBytes,
    v.masterKey,
    true,
  );
...
async function handleOpaqueFinishCredentialReset(
  ...
  const { registrationRecord, exportKey } = opaqueClient.finishRegistration({
    ...
    clientRegistrationState: op.clientState,
    identifiers: {
      client: clientIdentity(op.accountId, false),
      server: SERVER_IDENTITY,
    },
    keyStretching: KEY_STRETCHING,
  });
  ...
  if (prev) {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "keystore: opaquePasswordChangeCommit cleanup failed",
          delErr,
        );
      }
    }
    ...
    switch (msg.cmd) {
      ...
      case "prepareLogin":
        respond(
          port,
          msg.id,
          await handlePrepareLogin(msg.args as PrepareLoginArgs),
        );
        break;
      case "verifyLoginProof":
        respond(
          port,
          msg.id,
          handleVerifyLoginProof(msg.args as VerifyLoginProofArgs),
        );
        break;
      case "completeLoginUnlock":
        respond(
          port,
          msg.id,
          await handleCompleteLoginUnlock(msg.args as CompleteLoginUnlockArgs),
        );
        break;
      case "abandonLogin":
        handleAbandonLogin();
      ...
      case "opaqueStartRegistration":
        respond(
          port,
          msg.id,
          await handleOpaqueStartRegistration(
            msg.args as OpaqueStartRegistrationArgs,
          ),
        );
        break;
      case "opaqueFinishRegistration":
        respond(
          port,
          msg.id,
          await handleOpaqueFinishRegistration(
            msg.args as OpaqueFinishRegistrationArgs,
          ),
        );
        break;
      case "opaqueFinalizeRegister":
        respond(
          port,
          msg.id,
          await handleOpaqueFinalizeRegister(
            msg.args as OpaqueFinalizeRegisterArgs,
          ),
        );
        break;
      case "opaqueStartAuth":
        respond(
          port,
          msg.id,
          await handleOpaqueStartAuth(msg.args as OpaqueStartAuthArgs),
        );
        break;
      case "opaqueFinishAuth":
        respond(
          port,
          msg.id,
          await handleOpaqueFinishAuth(msg.args as OpaqueFinishAuthArgs),
        );
        break;
      case "opaqueCompleteLoginUnlock":
        respond(
          port,
          msg.id,
          await handleOpaqueCompleteLoginUnlock(
            msg.args as OpaqueCompleteLoginUnlockArgs,
          ),
        );
        break;
      ...
      case "migrationStartRegistration":
        respond(
          port,
          msg.id,
          await handleMigrationStartRegistration(
            msg.args as MigrationStartRegistrationArgs,
          ),
        );
        break;
      case "migrationFinishStage":
        respond(
          port,
          msg.id,
          await handleMigrationFinishStage(
            msg.args as MigrationFinishStageArgs,
          ),
        );
        break;
      case "opaqueRecoverySetupStart":
        respond(
          port,
          msg.id,
          await handleOpaqueRecoverySetupStart(
            msg.args as OpaqueRecoverySetupStartArgs,
          ),
        );
        break;
      case "opaqueRecoverySetupFinish":
        respond(
          port,
          msg.id,
          await handleOpaqueRecoverySetupFinish(
            msg.args as OpaqueRecoverySetupFinishArgs,
          ),
        );
        break;
      case "opaqueCompleteRecoveryUnlock":
        respond(
          port,
          msg.id,
          await handleOpaqueCompleteRecoveryUnlock(
            msg.args as OpaqueCompleteRecoveryUnlockArgs,
          ),
        );
        break;
      case "opaquePrepareCredentialReset":
        respond(
          port,
          msg.id,
          await handleOpaquePrepareCredentialReset(
            msg.args as OpaquePrepareCredentialResetArgs,
          ),
        );
        break;
      case "opaqueFinishCredentialReset":
        respond(
          port,
          msg.id,
          await handleOpaqueFinishCredentialReset(
            msg.args as OpaqueFinishCredentialResetArgs,
          ),
        );
        break;
      case "opaquePrepareAmkRotation":
        respond(
          port,
          msg.id,
          await handleOpaquePrepareAmkRotation(
            msg.args as OpaquePrepareAmkRotationArgs,
          ),
        );
        break;
      case "opaqueFinishAmkRotation":
        respond(
          port,
          msg.id,
          await handleOpaqueFinishAmkRotation(
            msg.args as OpaqueFinishAmkRotationArgs,
          ),
        );
        break;
      case "opaquePasswordChangeStart":
        respond(
          port,
          msg.id,
          await handleOpaquePasswordChangeStart(
            msg.args as OpaquePasswordChangeStartArgs,
          ),
        );
        break;
      case "opaquePasswordChangeFinish":
        respond(
          port,
          msg.id,
          await handleOpaquePasswordChangeFinish(
            msg.args as OpaquePasswordChangeFinishArgs,
          ),
        );
        break;
      case "opaquePasswordChangeCommit":
        respond(
          port,
          msg.id,
          await handleOpaquePasswordChangeCommit(
            msg.args as OpaquePasswordChangeCommitArgs,
          ),
        );
        break;
      case "prepareRecoverySetup":
        respond(
          port,
          msg.id,
          await handlePrepareRecoverySetup(
            msg.args as PrepareRecoverySetupArgs,
          ),
        );
        break;
      case "prepareRecoveryLogin":
        respond(
          port,
          msg.id,
          await handlePrepareRecoveryLogin(
            msg.args as PrepareRecoveryLoginArgs,
          ),
        );
        break;
      case "verifyRecoveryProof":
        respond(
          port,
          msg.id,
          handleVerifyRecoveryProof(msg.args as VerifyRecoveryProofArgs),
        );
        break;
      case "completeRecoveryUnlock":
        respond(
          port,
          msg.id,
          await handleCompleteRecoveryUnlock(
            msg.args as CompleteRecoveryUnlockArgs,
          ),
        );
        break;
      case "prepareCredentialReset":
        respond(
          port,
          msg.id,
          await handlePrepareCredentialReset(
            msg.args as PrepareCredentialResetArgs,
          ),
        );
        break;
      ...
      case "prepareDeletionProof":
        respond(
          port,
          msg.id,
          await handlePrepareDeletionProof(
            msg.args as PrepareDeletionProofArgs,
          ),
        );
        break;
      case "preparePasswordChangeProof":
        respond(
          port,
          msg.id,
          await handlePreparePasswordChangeProof(
            msg.args as PreparePasswordChangeProofArgs,
          ),
        );
        break;
      case "verifyPasswordChangeProof":
        respond(
          port,
          msg.id,
          handleVerifyPasswordChangeProof(
            msg.args as VerifyPasswordChangeProofArgs,
          ),
        );
        break;
      ...
      case "commitPasswordChange":
        respond(
          port,
          msg.id,
          await handleCommitPasswordChange(
            msg.args as CommitPasswordChangeArgs,
          ),
        );
        break;
      case "invalidatePersistedVault":
        await handleInvalidatePersistedVault(
          msg.args as InvalidatePersistedVaultArgs,
        );
        respond(port, msg.id, undefined);
        break;
      ...
      case "enrollPersistent":
        respond(
          port,
          msg.id,
          await handleEnrollPersistent(msg.args as EnrollPersistentArgs),
        );
        break;
      case "tryRestoreFromPersistent":
        respond(
          port,
          msg.id,
          await handleTryRestoreFromPersistent(
            msg.args as TryRestoreFromPersistentArgs,
          ),
        );
        break;
      ...
      case "getPublicKey":
        respond(
          port,
          msg.id,
          await handleGetPublicKey(msg.args as GetPublicKeyArgs),
        );
        break;
      ...
      case "commitReformattedKey":
        respond(
          port,
          msg.id,
          await handleCommitReformattedKey(
            msg.args as CommitReformattedKeyArgs,
          ),
        );
        break;
      case "encrypt":
      ...
      case "encryptToKeys":
        respond(
          port,
          msg.id,
          await handleEncryptToKeys(msg.args as EncryptToKeysArgs),
        );
        break;
      default:
