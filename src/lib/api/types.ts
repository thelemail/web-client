...
export type TwoFactorVerifyResponse = {
	scope: 'login' | 'recovery' | 'password_change' | 'migration_stage' | 'migration_finalize';
	accountId: string;
	authScheme?: 'srp_v1' | 'opaque_v1';
...
export interface PasswordChangeVerifyResponse extends Partial<PasswordChangeGrant> {
	serverProof: string;
	twoFactor?: TwoFactorPending;
}
