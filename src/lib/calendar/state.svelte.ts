...
import { dayLabel, minutes, shortTime } from './format';
...
class CalendarState {
	...
	get visibleItems(): Item[] {
		return ITEMS.filter(
			(item) => this.calendarOn[item.cal]
		);
	}

	get visibleAllDay(): AllDayItem[] {
		return ALL_DAY.filter((item) => this.calendarOn[item.cal]);
	}
	...
	get taskGroups() {
		return TASK_GROUPS.map((name) => ({
			name,
			rows: TASKS.filter((task) => task.group === name).map(
				(task) => ({
					...task,
